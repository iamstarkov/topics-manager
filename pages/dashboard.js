...
const removeTopic = (token, repo, topics, topic) => {
  api.preview.put(`/repos/${repo.full_name}/topics`, token, {
...
class PageDashboard extends React.Component {
  static async getInitialProps(ctx) {
    ...
    const topics = await Promise.all(repos.map(x => api.preview.get(`/repos/${x.full_name}/topics`, token)));
        ...
        <ul>
          {repos.map((repo, i) => (
            <li key={repo.id}>
              ...
              {' '}
               <button title="add a list" onClick={() => {
                 const rawNewTopics = prompt(`Add topics to "${repo.full_name}", (separate several by comma)`);
                 const newTopics = rawNewTopics.split(',').map(x => x.trim()).filter(Boolean)
                 addTopics(token, repo, topics[i], newTopics)
              }}>+</button>
              {topics[i].names.length !== 0 && (<ul>
                  {topics[i].names.map(topic => (
                    <li key={topic}>
                      <button title="rename" onClick={() => {
                        const oldTopic = topic;
                        const newTopic = prompt(`Rename topic "${oldTopic}" of a "${repo.full_name}"`, oldTopic)
                        renameTopic(token, repo, topics[i], oldTopic, newTopic)
                      }}>✎</button>
                      {' '}
                      {topic}
                      {' '}
                  ...
                  <li key="remove-all">
                    <button title="remove all topics" onClick={() => {
                      removeAllTopics(token, repo);
                    }}>💥 remove all topics</button>
                  </li>
                </ul>)
              }
            </li>
